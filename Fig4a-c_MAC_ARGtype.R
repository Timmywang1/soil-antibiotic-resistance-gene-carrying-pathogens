...
df<- read.csv("AP_Genetype2_MAC.csv", header= T,row.names=1,quote="",check.names = T)##
head(df)
#typeof(df$ABCtransporter)

ARGtype<-df
ARGtype.active <- df[1:144, 4:23]

head(ARGtype.active[, 1:6], 3)

res.mca <- MCA(ARGtype.active, graph = FALSE)
...
fviz_mca_ind(res.mca, 
             ...
             habillage = "vancomycin", # color by groups 
             palette = c("#00AFBB", "#E7B800"),
             addEllipses = TRUE, ellipse.type = "confidence",
             ggtheme = theme_minimal())


fviz_ellipses(res.mca, c("kasugamycin","polymyxin"), 
              geom = "point")


res.mca <- MCA(ARGtype, quali.sup = 1:3,
